/*Ex5.1. Hãy chuyển một số ở hệ 10 thành số ở hệ cơ số b bất kì (1< b ≤ 36).*/
...
int main() {
	...
	scanf_s("%d", &n); // nhan vao gia tri so bo test
	while (n--) { //lap n lan
		scanf_s("%d%d", &k, &b); // nhap gia tri he 10, co so b
		if (k == 0) {
			printf("0\n");
		}
		else {
			...
			s[i] = '\0'; // cat duoi
			_strrev(s); // dao nguoc ket qua=> duoc kq chinh thuc
